...
class Panteao extends EventEmitter {
    constructor(options = {}) {
        ...
        this.useJarFallback = options.useJarFallback !== undefined ? options.useJarFallback : true;
    ...
    async start() {
        ...
        } else if (this.useJarFallback && fs.existsSync(jarPath)) {
            spawnCmd = 'java';
            spawnArgs = ['-jar', jarPath, this.project, '--port', String(this.port)];
        } else {
            if (!this.useJarFallback) {
                throw new Error(`Native engine executable not found at ${nativePath} and JAR fallback is disabled.`);
            } else {
