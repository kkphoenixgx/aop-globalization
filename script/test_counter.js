...
engine.on('ready', () => {
    console.log("[Node App] Conectado ao motor BDI e MAS pronto!");
    ...
    const intervalId = setInterval(() => {
        ...
        engine.sendMsg('tell', 'external', 'counter_agent', `counter(${counter})`);
        
        // Remove a percepção anterior para manter a base de crenças limpa
        if (counter > 1) {
            engine.sendMsg('untell', 'external', 'counter_agent', `counter(${counter - 1})`);
        }
    }, 500);
...
engine.on('action', (agent, action, callback) => {
    ...
    if (action === 'reset_counter') {
        ...
        engine.sendMsg('untell', 'external', 'counter_agent', `counter(${counter})`);
        
        // Reseta o contador local
        counter = 0;
        
        // Adiciona nova percepção
        engine.sendMsg('tell', 'external', 'counter_agent', `counter(0)`);
